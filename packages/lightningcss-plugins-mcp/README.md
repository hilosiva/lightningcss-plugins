...
- [lightningcss-plugin-fluid](https://www.npmjs.com/package/lightningcss-plugin-fluid) — `fluid()` 関数で流体タイポグラフィを実現（`snap` / `fit` / `free-max` / `free-min` / `free` キーワード対応）
...
| `get_fluid_options` | `minViewPort` / `maxViewPort` / `baseFontSize` / `unit` / `minCompSize` / `maxCompSize` / `mode` の詳細 |
| `get_fluid_functions` | `fluid()` の引数・キーワード・出力パターン・使用例 |
